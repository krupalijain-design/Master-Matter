          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={collectionHistory} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                ...
                <Line type="monotone" dataKey="billed" stroke="hsl(var(--accent))" strokeWidth={2} dot={{ r: 3 }} name="Billed" />
                <Line type="monotone" dataKey="collected" stroke="hsl(var(--success))" strokeWidth={2} dot={{ r: 3 }} name="Collected" />
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className="flex items-center gap-4 text-[11px] text-muted-foreground mt-2">
            <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-accent" />Billed</span>
            <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-success" />Collected</span>
          </div>
        </Card>
      </section>
...
function Kpi({ icon, label, value, delta, warning, info, footnote, onClick, onSaveAsReport }: {
  ...
  return (
    <Card
      className={"p-3 cursor-pointer hover:border-accent/60 transition-colors " + (warning ? "border-warning/40" : "")}
      onClick={onClick}
    >
